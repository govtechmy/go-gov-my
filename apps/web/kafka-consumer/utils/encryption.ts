 ...
 */
export async function encryptOutboxSecrets(values: Record<string, string>): Promise<string> {
...
async function decryptOutboxSecrets(ciphertext: string): Promise<Record<string, string>> {
  const result = await kms.send(
    new DecryptCommand({
      KeyId: ENCRYPTION_KEY_ID,
...
function interpolateSecrets(payload: string, secrets: Record<string, string>): string {
