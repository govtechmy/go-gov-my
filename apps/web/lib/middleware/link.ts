...
export default async function LinkMiddleware(req: NextRequest, ev: NextFetchEvent) {
  ...
  const { id, url, password, proxy, expiresAt, ios, android, geo, expiredUrl } = linkData;
  ...
  if (expiresAt && new Date(expiresAt) < new Date()) {
    if (expiredUrl) {
      ...
      return NextResponse.rewrite(new URL(`/expired/${domain}`, req.url), DUB_HEADERS);
    }
  }
  ...
  if (!(req.headers.get('dub-no-track') || searchParams.get('dub-no-track') === '1')) {
    try {
