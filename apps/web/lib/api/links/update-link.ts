...
export async function updateLink({
  ...
  let { id, domain, key, url, expiresAt, title, description, image, proxy, geo } = updatedLink;
  ...
  const response = await prisma.link.update({
    ...
    data: {
      ...
      image:
        proxy && image && !isStored(image) ? `${process.env.STORAGE_BASE_URL}/images/${id}` : image,
      utm_source,
      utm_medium,
      utm_campaign,
      utm_term,
    ...
    waitUntil(
      Promise.all([
        ...
        redis.hset(updatedLink.domain.toLowerCase(), {
          [updatedLink.key.toLowerCase()]: JSON.stringify(await formatRedisLink(response)),
        }),
        // if key is changed: delete the old key in Redis
        (changedDomain || changedKey) && redis.hdel(oldDomain.toLowerCase(), oldKey.toLowerCase()),
