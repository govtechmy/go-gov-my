...
export default function DateRangeSlider({ startDate, endDate, onRangeChange }: Props) {
  ...
  const updateSlider = () => {
    ...
    const selectedTrack = sliderGroup
      .append('line')
    ...
    // Tooltip
    const tooltip = sliderGroup.append('g').attr('class', 'tooltip').style('display', 'none');
    ...
    // Create handles
    const startHandle = sliderGroup
      .append('g')
      .attr('class', 'handle start-handle')
      .style('cursor', 'pointer');

    const endHandle = sliderGroup
      .append('g')
    ...
    function updateHandles() {
      ...
      if (hoveredHandle) {
        ...
        tooltip.style('display', 'block').attr('transform', `translate(${hoveredX - 50}, -45)`);
    ...
    const dragStart = d3.drag<SVGGElement, unknown>().on('drag', function (event) {
    ...
    const dragEnd = d3.drag<SVGGElement, unknown>().on('drag', function (event) {
