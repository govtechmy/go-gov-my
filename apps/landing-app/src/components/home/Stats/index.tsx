  ...
  return (
    ...
    >
      <div className="col-span-full flex flex-col items-center">
        <img src="/logo.svg" width={32} height={32} alt="logo" className={cn('object-contain')} />
      </div>
      ...
      >
        ...
        >
          <StatGroup className={cn('col-start-1 col-end-2')}>
            <StatTag icon={<IconPeople />} text={props.segments?.publicOfficers || ''} />
            <StatNumber
              size="large"
          ...
          <StatGroup className={cn('col-start-2 col-end-3 row-start-1')}>
            <StatTag icon={<IconLink />} text={props.segments?.linksCreated || ''} />
            <StatNumber
              size="large"
          ...
          >
            <StatTag icon={<IconCursor />} text={props.segments?.clicksServed || ''} />
            <StatNumber
              size="large"
              number={data.clicksServed}
              className={cn('mt-[0.25rem] lg:mt-[0.5rem]', 'ml-[1.5rem] md:ml-0')}
            />
          </StatGroup>
        </div>
        <div className={cn('col-span-full', 'mt-[3rem]')}>
          ...
          <div className={cn('mt-[2rem]', 'flex flex-row flex-nowrap items-center')}>
            {counters.map((counter, i) => (
              <div
                className={cn('p-[1rem]', 'flex flex-col items-start', i > 0 && 'ml-[2.5rem]')}
                key={i}
              >
                <StatLabel
                  className="text-sm"
                  text={counter.name === 'daily' ? props.counters.daily : props.counters.total}
                ></StatLabel>
                <StatNumber size="small" prefix={counter.prefix} number={counter.count} />
              </div>
            ))}
          </div>
          <LineChart
