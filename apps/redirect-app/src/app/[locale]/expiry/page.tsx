...
export async function generateMetadata({ params: { locale } }: MetadataProps): Promise<Metadata> {
  ...
  const title = t('metadata.expired.title');
  const description = t('metadata.expired.description');
...
export default async function PageLinkExpired({ params: { locale } }: PageProps) {
  ...
  return (
    <Page>
      ...
      <div className={cn('grid grid-cols-12 grid-rows-3')}>
        <Heading level={2} className={cn('col-span-full', 'text-center', 'mt-[1.5rem]')}>
          {t('pages.expired.title')}
        </Heading>
        <Paragraph
          textAlign="center"
          className={cn('col-span-full md:col-start-4 md:col-end-10', 'mt-[1rem]')}
        >
          {t('pages.expired.description')}
        </Paragraph>
        <Paragraph
          textSize="small"
          textAlign="center"
        ...
        >
          {t.rich('pages.common.redirect.description', {
            duration: (chunks) => <Redirect url={URL} countInSeconds={REDIRECT_SECONDS} />,
          })}
        </Paragraph>
      </div>
    </Page>
