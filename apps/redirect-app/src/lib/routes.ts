...
const parse = (obj: typeof routes, deepKey: DeepKeys<typeof routes>): string => {
...
const serializeQuery = (data: Record<string, any>) => {
  return Object.entries(data).reduce((prev, [key, value], index) => {
    return prev.concat(...[key, '=', value, Object.keys(data).length - 1 === index ? '' : '&']);
  }, '?');
};
...
const route = (
  ...
  if (query) {
    template = Object.entries(query).reduce((prev, [key, value], index) => {
      return prev.concat(...[key, '=', value, Object.keys(query).length - 1 === index ? '' : '&']);
    }, template + '?');
  }
