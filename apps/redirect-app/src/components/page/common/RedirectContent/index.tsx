...
export default function RedirectContent(props: Props) {
  ...
  return (
    <div className={cn('flex grow flex-col items-center justify-start tall:justify-center')}>
      ...
      />
      <div className={cn('mt-[2.25rem] md:mt-[2.5rem]', 'flex flex-col items-center')}>
        <ButtonB
          variant="primary"
          size="large"
        ...
        <Paragraph textAlign="center" textSize="small" className={cn('mt-[1.125rem] md:mt-[1rem]')}>
          {t.rich('pages.common.redirect.description', {
            duration: (chunks) => (
              <Redirect url={props.redirectUrl} countInSeconds={props.redirectSeconds} />
            ),
          })}
        </Paragraph>
      </div>
